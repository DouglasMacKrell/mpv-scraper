...
def detect_letterboxing(video_path: Path) -> Optional[CropInfo]:
    ...
    try:
        ...
        if abs(current_aspect - 16 / 9) < 0.1:  # Allow small tolerance
            ...
            logger.info(
                f"Detected letterboxing in {video_path.name}: "
...
def crop_video_to_4_3(
    input_path: Path, output_path: Path, crop_info: CropInfo, quality: str = "high"
) -> bool:
    """
    ...
    try:
        ...
        logger.error(
            "ffmpeg not found. Please install ffmpeg to enable video cropping."
        )
        return False
...
def batch_crop_videos_to_4_3(
    ...
    for video_file in directory.iterdir():
        if (
            not video_file.is_file()
            or video_file.suffix.lower() not in video_extensions
        ):
            continue
        # Skip AppleDouble files and already-cropped outputs
        if video_file.name.startswith("._"):
            continue
        if video_file.stem.endswith("_4x3"):
            continue
        ...
        try:
            ...
            logger.warning(f"Detection failed for {video_file.name}: {e}")
            continue
        if not crop_info:
            continue
        ...
        # Skip if output already exists unless overwrite requested
        if output_path.exists() and not overwrite:
            logger.info(
                f"Skipping {video_file.name} - {output_path.name} already exists"
            )
            continue

        if dry_run:
            logger.info(
                f"[DRY RUN] Would crop {video_file.name} "
