...
def convert_mkv_to_mp4(
    ...
    try:
        ...
        logger.info(
            f"Converting {input_path.name} to MP4 using {preset.name} preset..."
        )
...
def batch_convert_mkv_to_mp4(
    ...
    for mkv_file in directory.glob("*.mkv"):
        ...
        if output_path.exists() and not overwrite:
            logger.warning(
                f"Skipping {mkv_file.name} - {output_path.name} already exists"
            )
            continue

        if dry_run:
            logger.info(
                f"[DRY RUN] Would convert {mkv_file.name} -> {output_path.name} "
...
def batch_convert_mkv_to_mp4_with_fallback(
    directory: Path, dry_run: bool = False, overwrite: bool = False
) -> Tuple[int, int]:
    """
    ...
    for mkv_file in directory.glob("*.mkv"):
        ...
        if convert_mkv_to_mp4(mkv_file, output_path, VANILLA_WITH_SUBS, overwrite):
            ...
            logger.warning(
                f"Failed to convert {mkv_file.name} with subtitles, trying without subtitles..."
            )
            if convert_mkv_to_mp4(mkv_file, output_path, VANILLA_NO_SUBS, overwrite):
                successful += 1
                logger.info(
                    f"✓ Successfully converted {mkv_file.name} without subtitles (fallback)"
                )
            else:
                logger.error(
                    f"Failed to convert {mkv_file.name} with both subtitle options"
                )
